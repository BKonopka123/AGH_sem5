...
insert into czytelnik(imie, nazwisko) values ('Maciej', 'Wokulski');
...
CREATE OR REPLACE FUNCTION get_pozyyczone_ksiazki(id_czytelnika INT)
RETURNS TABLE (
    tytul VARCHAR(255),
    oddana BOOLEAN
)
...
BEGIN
    RETURN QUERY
    SELECT k.tytul, 
           CASE WHEN w.data_zwrotu IS NULL THEN FALSE ELSE TRUE END AS oddana
    FROM ksiazki k
    JOIN wc ON wc.ksiazka_id = k.ksiazka_id
    JOIN wypozyczenie w ON w.wypozyczenie_id = wc.wypozyczenie_id
    WHERE w.czytelnik_id = id_czytelnika;
END;
$$ LANGUAGE plpgsql;

SELECT * FROM get_pozyyczone_ksiazki(1);